...
if (keyboard_check(vk_f9) || keyboard_check(ord("X")) || keyboard_check(vk_shift))
    sprint = 1.5;
...
if (movement == 1) {
    if (obj_time.left) {
        ...
        if (global.debug == 1 && keyboard_check(vk_backspace))
            x -= 5;
    ...
    if (obj_time.right && obj_time.left == 0) {
        turned = 1;
    ...
    if (obj_time.down && obj_time.up == 0) {
        turned = 1;
        y += (3 * sprint);
...
crumpet = (collision_point(bbox_left - 3, bbox_top - 3, obj_solidparent, 0, 1) == -4) ? 1 : 2;
strumpet = bbox_top;
trumpet = bbox_left;
...
if (!instance_exists(obj_battler)) {
    ...
    if (global.flag[85] == 1 && dsprite == 1104)
